...
class Role(db.Model):
    __tablename__='roles' #注意这个talblename！！！！
    #代表数据库中使用的表的名称
    id = db.Column(db.Integer,primary_key=True)
    name = db.Column(db.String(64),unique=True)
    default = db.Column(db.Boolean,default=False,index=True)
    permissions = db.Column(db.Integer)
    users = db.relationship('User',backref='role',lazy='dynamic')#在User模型中添加一个role属性，从而定义反向关系
    #id等类变量为db.Column类中的实例，是该模型（可以指class Role等类）的属性
    #primary_key=True代表id（列或者说属性的类型）为该表的主键（其值能唯一的标识表中的某一行）
    #unique=True代表该列不准出现重复值
    def __repr__(self):
...
class User(db.Model,UserMixin):
    __tablename__ = 'users'
    role_id = db.Column(db.Integer,db.ForeignKey('roles.id'))#foreignkey注意tablename
    id = db.Column(db.Integer,primary_key=True)
    username = db.Column(db.String(64),unique=True,index=True)
    #index=True表示为username列创建索引，提升查询效率
    ...
    def confirm(self,token):
        ...
        try:
            ...
            return False
        if data.get('confirm') != self.id: #data是一个dict，confirm是其中一个key，其值为id
            return False
        self.confirmed = True
        db.session.add(self)
        return True
